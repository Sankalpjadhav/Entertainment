...
<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST")
   {
      ...
      if($category == 'comedian')
      {
        $sql = "SELECT *
        FROM comediandb
        WHERE comediandb.bMobNo='$number'";
        $result = mysqli_query($connection,$sql);
        $count = mysqli_num_rows($result);
        if($count == 1)
        {
          $sql="UPDATE comediandb SET comediandb.password='$password' where comediandb.bMobNo='$number'";
          $result = mysqli_query($connection,$sql);
      ...
      if($category == 'audience')
      {
        $sql = "SELECT *
        FROM audiencedb
        WHERE audiencedb.mbNo='$number' ";
        $result = mysqli_query($connection,$sql);
        $count = mysqli_num_rows($result);
        if($count == 1)
        {

          $sql="UPDATE audiencedb SET audiencedb.password='$password' where audiencedb.mbNo='$number'";
          $result = mysqli_query($connection,$sql);
        }
